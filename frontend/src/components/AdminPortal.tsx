...
import type { Message, JournalEntry, Exercise, Soundscape, VisualizationType } from '../../../types';
...
const AdminPortal: React.FC = () => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('Failed to add exercise:', errData);
      return;
    }

    const createdEx = await res.json();
    addExercise(createdEx); // Update local state
    setExTitle('');
    setExDesc('');
    setExSteps('');
  } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col">
      {/* Admin Header */}
      <header className="bg-slate-800 text-white p-4 shadow-md flex justify-between items-center z-10 max-w-6xl mx-auto w-full">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* MAIN CONTENT AREA */}
        <div className="flex-1 overflow-y-auto bg-slate-50 p-6 max-w-6xl mx-auto w-full">
